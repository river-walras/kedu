...
配额实测(2026-06):get_all_securities(['index'])=732 只(554 在市/178 退市),活跃指数 ~97% 非空、
均 ~156 只成分;get_index_stocks/get_index_weights **按成分数计费**(300 只成分=300 配额/次,空指数=0)。
故成分种子是大头(~数千万配额 + ~数十万次往返,需跨次 min_spare 续传),估值/日线很便宜。
因绝大多数锚点无变更却仍按成分全额计费,**步长 MEMBER_STEP 是配额主杠杆**(见其定义);空指数计 0
配额,无需对空指数特判。
...
# 成分二分的粗网格步长(交易日)。get_index_stocks 按**成分数计费**(300 只成分=300 配额/次),
# 而绝大多数锚点是「无变更」的重复采样,故步长是种子配额的**主杠杆**:
# 21≈月度(基线)/42≈双月(约½配额,默认)/63≈季度(约⅓)。调仓≤季度且递归二分能在更宽
# 区间内定位多个变更日,故 42 安全;唯一盲点是「同一区间内加入又移除且两端集合相同」的瞬变
# (调度指数几乎不存在)。要更精确可调小,要更省配额可调大。
MEMBER_STEP = 42
